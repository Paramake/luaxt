...
static int f_set_cursor(lua_State* L) {
    ...
    if (!cursor) {
        cursor = SDL_CreateSystemCursor(static_cast<SDL_SystemCursor>(n));
...
static int f_list_dir(lua_State* L)
{
    ...
    {
        lua_pushstring(L, (it.path().filename()).string().c_str());
        lua_rawseti(L, -2, i++);
    }
...
int InitializeSystem(lua_State* L)
{
	const luaL_Reg lib[] =
	{
